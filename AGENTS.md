...
- **GitLab** (`glab`): `mr list --output json` does **not** include `head_pipeline`/pipeline info at all. Getting pipeline status requires one follow-up `mr view <iid> --output json` call per MR. `has_conflicts` (bool) is the correct conflict-only signal — don't reuse `detailed_merge_status` (it conflates conflicts, approvals, and other blockers). A GitLab instance's `glab auth login` credentials can be stored under a different hostname than the instance's real HTTPS/API host — e.g. verified live: `glab auth status` showed the token stored under an SSH-only hostname ("ssh.git.example.com") even though API calls go out over HTTPS to a different host ("git.example.com"). `GITLAB_HOST` passed to `glab` must match whichever hostname the credentials are actually stored under, or `glab` fails auth with no token found for that host — it is NOT simply "the real API host." See `GitLabInstanceConfig.ssh_host`, which both overrides `GITLAB_HOST` for this reason and lets `generate_myprojects` map a scanned SSH remote back to the right table. `glab mr checkout <n>` without `-f` correctly refuses with an error on a diverged local branch (safer default than gh's), but still needs `-f` to actually reset it.
